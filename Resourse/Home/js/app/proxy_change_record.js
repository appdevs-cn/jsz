...
!(function(win){
    var indexHandler = {
        ...
        ,LoadingAfter: function()
        {
            ...
            indexHandler.getProxyChangeRecord(),indexHandler.search()
        }
        ,Logout: function()
        {
            // 退出系统
        ...
        ,getProxyChangeRecord: function()
        {
            $data = $("#recordAddFrom").serialize();
            $.post("/getProxyChangeRecord",$data,function(data){
                if(data.length==0)
                {
                    $("#list").html(tpl('#empty_change_record'))
                    return false;
                }
                else
                {
                    $("#list").html(tpl('#change_record',{rows:[data]}))
                    indexHandler.TrHover()
                    $(".pageCls").on("click",function(){
                        $p.val($(this).attr("p"));
                        indexHandler.getProxyChangeRecord();
                    })
                    return false;
                }
            },'json')
