...
!(function(win){
    var indexHandler = {
        ...
        ,LoadingAfter: function()
        {
            ...
            indexHandler.getPtReport(),indexHandler.search()
        }
        ,Logout: function()
        {
            // 退出系统
        ...
        ,getPtReport: function()
        {
            $data = $("#ProxyPtReportFrom").serialize();
            $.post("/ProxySearchPtReport",$data,function(data){
                if(data.length==0)
                {
                    $("#list").html(tpl('#empty_proxy_pt_report'))
                    return false;
                }
                else
                {
                    $("#list").html(tpl('#proxy_pt_report',{rows:[data]}))
                    indexHandler.TrHover()
                    $(".pageCls").on("click",function(){
                        $p.val($(this).attr("p"));
                        indexHandler.getPtReport();
                    })
                    return false;
                }
            },'json')
