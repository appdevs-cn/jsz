...
!(function(win){
    var indexHandler = {
        ...
        ,LoadingAfter: function()
        {
            ...
            indexHandler.getAgRecord(),indexHandler.search()
        }
        ,Logout: function()
        {
            // 退出系统
        ...
        ,getAgRecord: function()
        {
            $data = $("#ProxyAgFrom").serialize();
            $.post("/ProxySearchAgRecord",$data,function(data){
                if(data.length==0)
                {
                    $("#list").html(tpl('#empty_proxy_ag_record'))
                    return false;
                }
                else
                {
                    $("#list").html(tpl('#proxy_ag_record',{rows:[data]}))
                    indexHandler.TrHover()
                    $(".pageCls").on("click",function(){
                        $p.val($(this).attr("p"));
                        indexHandler.getAgRecord();
                    })
                    return false;
                }
            },'json')
