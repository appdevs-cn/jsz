...
!(function(win){
    var indexHandler = {
        ...
        ,LoadingAfter: function()
        {
            ...
            indexHandler.getTransferDetail(),indexHandler.search()
        }
        ,Logout: function()
        {
            // 退出系统
        ...
        ,getTransferDetail: function()
        {
            $data = $("#transferDetailFrom").serialize();
            $.post("/getTransferDetail",$data,function(data){
                if(data.length==0)
                {
                    $("#list").html(tpl('#empty_transfer_list'))
                    return false;
                }
                else
                {
                    $("#list").html(tpl('#transfer_list',{rows:[data]}))
                    indexHandler.TrHover()
                }
                $(".pageCls").on("click",function(){
                    $p.val($(this).attr("p"));
                    indexHandler.getTransferDetail();
                })
                return false;
            },'json')
        }
