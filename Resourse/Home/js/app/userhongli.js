...
!(function(win){
    var indexHandler = {
        ...
        ,LoadingAfter: function()
        {
            ...
            indexHandler.getHongliRecord(),indexHandler.search()
        }
        ,Logout: function()
        {
            // 退出系统
        ...
        ,getHongliRecord: function()
        {
            $data = $("#hongliFrom").serialize();
            $.post("/SearchHongliRecord",$data,function(data){
                if(data.length==0)
                {
                    $("#list").html(tpl('#empty_hongli_record'))
                    return false;
                }
                else
                {
                    $("#list").html(tpl('#hongli_record',{rows:[data]}))
                    indexHandler.paifa()
                    indexHandler.TrHover()
                    $(".pageCls").on("click",function(){
                        $p.val($(this).attr("p"));
                        indexHandler.getHongliRecord();
                    })
                    return false;
                }
            },'json')
        ...
        ,paifa: function()
        {
            $('a[data-method="paifa_hongli"]').on('click', function(){
                $.post('/paifaHl',{"id":$(this).attr("pd")},function(info){
