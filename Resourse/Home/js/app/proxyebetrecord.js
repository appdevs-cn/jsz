...
!(function(win){
    var indexHandler = {
        ...
        ,LoadingAfter: function()
        {
            ...
            indexHandler.getEbetRecord(),indexHandler.search()
        }
        ,Logout: function()
        {
            // 退出系统
        ...
        ,getEbetRecord: function()
        {
            $data = $("#ProxyEbetFrom").serialize();
            $.post("/ProxySearchEbetRecord",$data,function(data){
                if(data.length==0)
                {
                    $("#list").html(tpl('#empty_proxy_ebet_record'))
                    return false;
                }
                else
                {
                    $("#list").html(tpl('#proxy_ebet_record',{rows:[data]}))
                    indexHandler.TrHover()
                    $(".pageCls").on("click",function(){
                        $p.val($(this).attr("p"));
                        indexHandler.getEbetRecord();
                    })
                    return false;
                }
            },'json')
