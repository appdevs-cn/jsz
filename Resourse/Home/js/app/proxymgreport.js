...
!(function(win){
    var indexHandler = {
        ...
        ,LoadingAfter: function()
        {
            ...
            indexHandler.getMgReport(),indexHandler.search()
        }
        ,Logout: function()
        {
            // 退出系统
        ...
        ,getMgReport: function()
        {
            $data = $("#ProxyMgReportFrom").serialize();
            $.post("/ProxySearchMgReport",$data,function(data){
                if(data.length==0)
                {
                    $("#list").html(tpl('#empty_proxy_mg_report'))
                    return false;
                }
                else
                {
                    $("#list").html(tpl('#proxy_mg_report',{rows:[data]}))
                    indexHandler.TrHover()
                    $(".pageCls").on("click",function(){
                        $p.val($(this).attr("p"));
                        indexHandler.getMgReport();
                    })
                    return false;
                }
            },'json')
