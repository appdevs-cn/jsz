!(function(win){
    var indexHandler = {
        ...
        {
            $.post('/getreplayContract',function(data){
                if(data.length==0)
                {
        ...
        {
            $('a[data-method="agree_contract"]').on("click", function(){
                var agreeid = $(this).attr("data-field")
                layui.use('layer', function(layer){
                var index = layer.confirm('<div style="text-align:center">您确定要跟上级进行分红签约吗?</div>', {
                    btn: ['确定签约','取消签约']
                    ,anim:3
                    ,btnAlign: 'c'
                    }, function(){
                        $.post("/agree",{"agreeid":agreeid},function(info){
                            layer.close(index)
                            if(info)
                            {
        ...
        {
            $('a[data-method="relieve_contract"]').on("click", function(){
                var agreeid = $(this).attr("data-field")
                layui.use('layer', function(layer){
                var index = layer.confirm('<div style="text-align:center">您确定要跟上级解除分红签约吗?</div>', {
                    btn: ['确定解约','取消解约']
                    ,anim:3
                    ,btnAlign: 'c'
                    }, function(){
                        $.post("/applyRelease",{"agreeid":agreeid},function(info){
                            layer.close(index)
                            if(info)
                            {
        ...
        ,AgreeReleaseContract: function()
        {
            $('a[data-method="agreeDisContract"]').on('click', function(){
                $.post("/downtotopagreeRelease",{"agreeid":$(this).attr("data-field")},function(info){
                    if(info)
                    {
