...
!(function(win){
    var indexHandler = {
        ...
        ,LoadingAfter: function()
        {
            ...
            indexHandler.getEbetReport(),indexHandler.search()
        }
        ,Logout: function()
        {
            // 退出系统
        ...
        ,getEbetReport: function()
        {
            $data = $("#ProxyEbetReportFrom").serialize();
            $.post("/ProxySearchEbetReport",$data,function(data){
                if(data.length==0)
                {
                    $("#list").html(tpl('#empty_proxy_ebet_report'))
                    return false;
                }
                else
                {
                    $("#list").html(tpl('#proxy_ebet_report',{rows:[data]}))
                    indexHandler.TrHover()
                    $(".pageCls").on("click",function(){
                        $p.val($(this).attr("p"));
                        indexHandler.getEbetReport();
                    })
                    return false;
                }
            },'json')
