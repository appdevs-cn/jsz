...
!(function(win){
    var indexHandler = {
        ...
        ,LoadingAfter: function()
        {
            ...
            indexHandler.getAgReport(),indexHandler.search()
        }
        ,Logout: function()
        {
            // 退出系统
        ...
        ,getAgReport: function()
        {
            $data = $("#ProxyAgReportFrom").serialize();
            $.post("/ProxySearchAgReport",$data,function(data){
                if(data.length==0)
                {
                    $("#list").html(tpl('#empty_proxy_ag_report'))
                    return false;
                }
                else
                {
                    $("#list").html(tpl('#proxy_ag_report',{rows:[data]}))
                    indexHandler.TrHover()
                    $(".pageCls").on("click",function(){
                        $p.val($(this).attr("p"));
                        indexHandler.getAgReport();
                    })
                    return false;
                }
            },'json')
