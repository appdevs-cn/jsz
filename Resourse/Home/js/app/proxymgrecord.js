...
!(function(win){
    var indexHandler = {
        ...
        ,LoadingAfter: function()
        {
            ...
            indexHandler.getMgRecord(),indexHandler.search()
        }
        ,Logout: function()
        {
            // 退出系统
        ...
        ,getMgRecord: function()
        {
            $data = $("#ProxyMgFrom").serialize();
            $.post("/ProxySearchMgRecord",$data,function(data){
                if(data.length==0)
                {
                    $("#list").html(tpl('#empty_proxy_mg_record'))
                    return false;
                }
                else
                {
                    $("#list").html(tpl('#proxy_mg_record',{rows:[data]}))
                    indexHandler.TrHover()
                    $(".pageCls").on("click",function(){
                        $p.val($(this).attr("p"));
                        indexHandler.getMgRecord();
                    })
                    return false;
                }
            },'json')
