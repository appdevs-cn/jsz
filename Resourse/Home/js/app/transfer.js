...
!(function(win){
    var indexHandler = {
        ...
        ,LoadingAfter: function()
        {
            ...
            indexHandler.getTransferRecord(),indexHandler.search()
        }
        ,Logout: function()
        {
            // 退出系统
        ...
        ,getTransferRecord: function()
        {
            $data = $("#TransferFrom").serialize();
            $.post("/searchTransfer",$data,function(data){
                if(data.length==0)
                {
                    $("#list").html(tpl('#empty_transfer_record'))
                    return false;
                }
                else
                {
                    $("#list").html(tpl('#transfer_record',{rows:[data]}))
                    indexHandler.TrHover()
                    $(".pageCls").on("click",function(){
                        $p.val($(this).attr("p"));
                        indexHandler.getTransferRecord();
                    })
                    return false;
                }
            },'json')
