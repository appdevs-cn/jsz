...
class TongHuiWxModel
{
    ...
    static function getInstance($money=0)
    {
        ...
        {
            self::$_instance = new TongHuiWxModel($money);
        }
    ...
    public function showOrder()
    {
        ...
        $onlinetool_rows=M('detail_onlinetool')->where(array("toolid"=>11,"status"=>1))->find();
        $data->MER_NO = $onlinetool_rows['clientID'];
        $data->CHARSET = "UTF-8";
        $data->BACK_NOTIFY_URL = "http://".$onlinetool_rows['url']."/th_wechart_server.php";
        $data->PAGE_NOTIFY_URL = "http://".$onlinetool_rows['url']."/th_wechart_respondret.php";
        $data->PAY_TYPE = 1;
...
class KeyValues
{
    ...
    function link()
    {
        ...
        URLUtils::appendParam($strb, AppConstants::$KEY, C("WECHARTAPPKEY"));
