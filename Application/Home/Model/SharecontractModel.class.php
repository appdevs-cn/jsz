...
class SharecontractModel extends Model
{
    protected $trueTableName = 'sharecontract';

    // 查询当前用户是否已经和上级签署了分红契约
    public function isHaveSharecontrace()
    {
    ...
    public function AddContractItem()
    {
        ...
        if(session("SESSION_ROLE")==3)
        {
            ...
            $maxGainRate = $res['maxgainrate'];
            $minGainRate = $res['mingainrate'];
        }
        ...
        if($firstparty!=$uReturn['parent_id'] || $uReturn['group_id']!=4 || bccomp($bonusLevel,12.7,2)==-1)
        {
            return "0-签订契约分红失败";
            exit();
        }
        ...
        // 查询待签约的用户与他的下级最高签约标准
        $maxreceive = $this->where(array("firstparty"=>$secondparty))->max('receive');
        $rewardarr = array();
        $isSuccess = true;
        for($i=0; $i<$len; $i++)
        {
            $receive = $addItem['tbreceive'][$i];
        ...
        for($i=0; $i<$len; $i++)
        {
            ...
            $loss = $addItem['tbloss'][$i];
            $receive = $addItem['tbreceive'][$i];
            if($dailysales=="" || $loss=="" || $receive=="") continue;

            if(session("SESSION_ROLE")==3)
            {
                if(bccomp($receive,$maxGainRate,2)==1 || bccomp($receive,$minGainRate,2)==-1)
                {
                    return "0-契约分红范围为15到25%,请核查所填分红标准！";
                    exit();
                }
            }
            ...
            "secondpartypath"=>$secondpartypath,"dailysales"=>$dailysales,"loss"=>$loss,"starttime"=>$starttime,
    ...
    public function SearchContracting()
    {
        ...
        $xReturn = $this->where(array("secondparty"=>$id,"status"=>1))->field('firstparty,secondpartypath,dailysales,loss,receive')->select();
    ...
    public function SearchContractend()
    {
        ...
        $addItem = $addItem['data'];
        $id = $_xDe->decode($addItem['contractId']);
        $xReturn = $this->where("secondparty=".$id." and (status=2 or status=3)")->field('id,firstparty,secondpartypath,dailysales,loss,receive,status')->select();
        $secondpartypath = explode(",", $xReturn[0]['secondpartypath']);
        $secondParentId = array_pop($secondpartypath);
        // 验证这条数据的合法性
        if($xReturn[0]['firstparty']!=session("SESSION_ID") || $secondParentId!=session("SESSION_ID"))
    ...
    public function SearchDisContracting()
    {
        ...
        $addItem = $addItem['data'];
        $id = $_xDe->decode($addItem['contractId']);
        $xReturn = $this->where(array("id"=>$id,"status"=>3))->field('id,firstparty,secondpartypath,originator,dailysales,loss,receive')->select();
        $secondpartypath = explode(",", $xReturn[0]['secondpartypath']);
        $secondParentId = array_pop($secondpartypath);
        // 验证这条数据的合法性
        if($xReturn[0]['firstparty']!=session("SESSION_ID") || $secondParentId!=session("SESSION_ID"))
    ...
    public function GetContract()
    {
        ...
        $_xDe=new \XDeode();
        $secondparty = session("SESSION_ID");
        $xReturn = $this->where(array("secondparty"=>$secondparty))->field('id,dailysales,loss,receive,originator,status')->select();
        $return = array();
        ...
        foreach($xReturn as $item)
        {
            ...
            if($item['status']==1)
            {
                ...
                $item['operate'] = '<a class="layui-btn" data-method="agree_contract" data-field="'.$item['id'].'">同意签约</a>';
            }
            else if($item['status']==2)
            {
                $item['desc'] = "<font style='color:green'>签约成功</font>";
                $item['operate'] = '<a class="layui-btn" data-method="relieve_contract" data-field="'.$item['id'].'">申请解约</a>';
            }
            else if($item['status']==3)
            {
                $item['desc'] = "<font style='color:#ee6528'>解除签约中...</font>";
                if($item['originator']==$secondparty)
                    $item['operate'] = '<a class="layui-btn" data-field="'.$item['id'].'" data-method="agreeDisContract">同意解约</a>';
                else
    ...
    public function agreeContractHandler()
    {
        ...
        if($this->where(array("id"=>$id))->save(array("status"=>2)))
        {
            ...
            $msg = "您的下级".$username."同意了一份契约分红，请查看";
            $send = $msg."-5-".time();
    ...
    public function releaseContractHandler()
    {
        ...
        {
            $username = M("user")->where(array("id"=>$secondparty))->getField("username");
            $msg = "您的下级".$username."申请解除一份契约分红，请查看";
            $send = $msg."-5-".time();
            $content = array('type'=>"inbox","to"=>$list['firstparty'],'content'=>$send);
            $result = self::sendContractWebSocketMsg($content);
            return true;
