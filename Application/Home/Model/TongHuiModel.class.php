...
class TongHuiModel
{
    ...
    static function getInstance($money=0, $bank='')
    {
        ...
        {
            self::$_instance = new TongHuiModel($money, $bank);
        }
    ...
    public function showOrder()
    {
        ...
        $onlinetool_rows=M('detail_onlinetool')->where(array("toolid"=>4,"status"=>1))->find();
        $data->MER_NO = $onlinetool_rows['clientID'];
        $data->CHARSET = "UTF-8";
        $data->BACK_NOTIFY_URL = "http://".$onlinetool_rows['url']."/tonghui_server.php";
        $data->PAGE_NOTIFY_URL = "http://".$onlinetool_rows['url']."/tonghui_respondret.php";
        $data->PAY_TYPE = 1;
...
class KeyValues
{
    ...
    function link()
    {
        ...
        URLUtils::appendParam($strb, AppConstants::$KEY, C("THAPPKEY"));
