...
class CommonController extends Controller
{
    ...
    public function _initialize()
    {
        ...
        // 分红开关
        $this->share_switch = $this->userInfo->share_switch;

        // 日工资开关
        $this->dayrate_switch = $this->userInfo->dayrate_switch;
    ...
    protected function CreateUserBonus($lotteryid = 0, $uid = '', $fd = true) {
        $rid = $uid == '' ? session("SESSION_ID") : $uid;
        $saveDataObj = new \Home\Controller\CommonSaveDataController();
        $userInfo = $saveDataObj::UserBonusById_Redis($rid, 600, 'select');
        $bonusArray = $userInfo["bonus"];
        $tempArray = array();
        foreach ($bonusArray as $key => $value) {
            if (is_array($value)) {
                switch (intval($key)) {
                    ...
                    case 33:
                    case 34:
                        $commonLotteryKey = "ssc_bonus";
                        $bdwLotteryKey = "ssc_bonus_bdw";
                        break;
                    case 5:
                    case 6:
                    case 7:
                    ...
                    case 24:
                    case 25:
                        $commonLotteryKey = "xw_bonus";
                        $bdwLotteryKey = "xw_bonus_bdw";
                        break;
                    case 9:
                    case 10:
                    case 26:
                    case 27:
                        $commonLotteryKey = "shf_bonus";
                        $bdwLotteryKey = "";
                        break;
                    case 11:
                    case 12:
                        $commonLotteryKey = "psds_bonus";
                        $bdwLotteryKey = "psds_bonus_bdw";
                        break;
                    case 13:
                    case 35:
                        $commonLotteryKey = "k8_bonus";
                        $bdwLotteryKey = "";
                        break;
                    case 14:
                        $commonLotteryKey = "shs_bonus";
                        $bdwLotteryKey = "";
                        break;
                    case 15:
                    case 16:
                        $commonLotteryKey = "k3_bonus";
                        $bdwLotteryKey = "";
                        break;
                    case 21:
                        $commonLotteryKey = "pk10_bonus";
                        $bdwLotteryKey = "";
                        break;
                    case 23:
                    case 28:
                    case 29:
                        $commonLotteryKey = "xy28_bonus";
                        $bdwLotteryKey = "";
                }
                $playway = C("LOTTERYTOWAY");
                $playwayArray = $playway[$key];
                foreach ($playwayArray as $waykey => $wayvalue) {
                    if (strpos($wayvalue, "不定位") !== FALSE) {
                        if ($bdwLotteryKey == "")
                            break;
                        $BonusArray = C($bdwLotteryKey);
                        foreach ($BonusArray as $bounskey => $bounsvalue) {
                            if ($bounskey == $value["bdw_bonus"]) {
                                foreach ($bounsvalue as $bounsnamekey => $bounsvaluekey) {
                                    if ($fd) {
                                        $tempArray[$key][$bounsnamekey] = $bounsvaluekey . '_' . $value["bdw_ret_point"];
                                    } else {
                                        $tempArray[$key][$bounsnamekey] = $bounsvaluekey;
                                    }
                                }
                            }
                        }
                        ...
                        if ($commonLotteryKey == "")
                            break;
                        $BonusArray = C($commonLotteryKey);
                        foreach ($BonusArray as $bounskey => $bounsvalue) {
                            if ($bounskey == $value["common_bonus"]) {
                                foreach ($bounsvalue as $bounsnamekey => $bounsvaluekey) {
                                    if (is_array($bounsvaluekey)) {
                                        if (strpos($bounsnamekey, "定单双") !== FALSE) {
                                            $html = "<dl>";
                                            foreach ($bounsvaluekey as $kk => $vv) {
                                                switch ($kk) {
                                                    case 0:
                                                        $pname = "0单5双";
                                                        break;
                                            ...
                                            $bounsvaluekey = $html;
                                        } else if (strpos($bounsnamekey, "猜中位") !== FALSE) {
                                            $html = "<dl>";
                                            foreach ($bounsvaluekey as $kk => $vv) {
                                                switch ($kk) {
                                                    case 0:
                                                ...
                                                $html .= "<dt>" . $cpname . ":" . $vv . "</dt>";
                                            }
                                            $html .= "</dl>";
                                            $bounsvaluekey = $html;
                                        } else if (strpos($bounsnamekey, "混合组选") !== FALSE || strpos($bounsnamekey, "组选和值") !== FALSE) {
                                            $html = "<dl>";
                                            foreach ($bounsvaluekey as $kk => $vv) {
                                                $html .= "<dt>" . $kk . ":" . $vv . "</dt>";
                                            }
                                            $html .= "</dl>";
                                            $bounsvaluekey = $html;
                                        } else if (strpos($bounsnamekey, "任选三") !== FALSE || strpos($bounsnamekey, "任选四") !== FALSE ||
                                            strpos($bounsnamekey, "任选五") !== FALSE || strpos($bounsnamekey, "任选六") !== FALSE ||
                                            strpos($bounsnamekey, "任选七") !== FALSE || strpos($bounsnamekey, "任选八") !== FALSE || strpos($bounsnamekey, "任选九") !== FALSE) {
                                            $html = "<dl>";
                                            foreach ($bounsvaluekey as $kk => $vv) {
                                                $html .= "<dt>" . $kk . ":" . $vv . "</dt>";
                                            }
                                            $html .= "</dl>";
                                            $bounsvaluekey = $html;
                                        } else if (strpos($bounsnamekey, "和值") !== FALSE || strpos($bounsnamekey, "三同号") !== FALSE ||
                                            strpos($bounsnamekey, "二同号") !== FALSE || strpos($bounsnamekey, "不同号") !== FALSE || strpos($bounsnamekey, "三连号") !== FALSE) {
                                            $html = "<dl>";
                                            foreach ($bounsvaluekey as $kk => $vv) {
                                                $html .= "<dt>" . $kk . ":" . $vv . "</dt>";
                                            }
                                            $html .= "</dl>";
                                            $bounsvaluekey = $html;
                                        } else if (strpos($bounsnamekey, "前三名组选复式") !== FALSE || strpos($bounsnamekey, "前三名组选单式") !== FALSE || strpos($bounsnamekey, "前二名组选复式") !== FALSE || strpos($bounsnamekey, "前二名组选单式") !== FALSE) {
                                            $html = "<dl>";
                                            foreach ($bounsvaluekey as $kk => $vv) {
                                                $html .= "<dt>" . $kk . ":" . $vv . "</dt>";
                                            }
                                    ...
                                    if ($fd) {
                                        $tempArray[$key][$bounsnamekey] = $bounsvaluekey . '_' . $value["common_ret_point"];
                                    } else {
                                        $tempArray[$key][$bounsnamekey] = $bounsvaluekey;
                                    }
                                }
