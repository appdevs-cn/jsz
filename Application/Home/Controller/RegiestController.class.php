...
class RegiestController extends Controller
{
    ...
    public function regiestHandler()
    {
        ...
        import("Class.XDeode");
        $_xDe=new \XDeode();
        $registid = $_xDe->decode($idmd5);
        $regist = M("regist")->where(array("id"=>$registid))->find();
        if(empty($regist)){
            echo "注册连接失效";
            exit();
        }
        if($regist["endtime"]<time()){
            echo "该注册链接已过期";
            exit();
        }
        ...
        $group = $regist["group"];
        $point = $regist["point"];
        $klcpoint = $regist["klcpoint"];
        $parentid = $regist["userid"];

        $exituser = M("user")->where(array("username"=>$username))->field("id")->find();
        if(!empty($exituser)){
            echo "该用户名已被注册,请另外更换一个";
            exit();
        }
        $userBonusInfo = M("user_bonus")->where(array('userid'=>$parentid))->find();
        $bonus_level = $userBonusInfo["bonus_level"];
        if(bccomp($point,$bonus_level,2)!=-1){
            echo "该链接返点设置不正确";
            exit();
        }
        ...
        if(bccomp($klcpoint,$klcbonuscontent,2)==1)
        {
            echo "幸运28的该链接返点设置不正确";
            exit();
        }
