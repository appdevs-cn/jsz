...
class Appdevs extends TagLib
{
    ...
    protected $tags = array(
        ...
        //游戏列表分页
        'gameList'         => array(
            'attr'  => 'gtype,typeid,arcid,titlelen,infolen,orderby,keyword,fieldflag,limit,pagesize,pageroll,pagetheme',
            'close' => 1,
        ),
        //活动列表分页
        'activityList'         => array(
            'attr'  => 'typeid,arcid,titlelen,infolen,orderby,keyword,fieldflag,limit,pagesize,pageroll,pagetheme',
            'close' => 1,
        ),
        //品牌列表分页
        'bandList'         => array(
...
str;
    ...
    //标签名前加下划线
    //游戏列表
    public function _gamelist($attr, $content) {
        ////非debug参属性参数只处理 一次
        $typeid   = !isset($attr['typeid']) || $attr['typeid'] == '' ? -1 : trim($attr['typeid']); //-1后面自动获取
        $arcid    = empty($attr['arcid']) ? '' : $attr['arcid']; //新增加20140413
        $titlelen = empty($attr['titlelen']) ? 0 : intval($attr['titlelen']);
        ...
        $keyword  = empty($attr['keyword']) ? '' : trim($attr['keyword']);
        $arcid = string2filter($arcid, ',', true);
        $gtype = I('gtype', 0, 'intval');
        $line = I('line', 0, 'intval');
...
<?php
	...
	if (\$_typeid>0 || substr(\$_typeid,0,1) == '$') {
		...
		\$ids = Common\Lib\Category::getChildsId(get_category(), \$_typeid, true);
		//p(\$ids);
		\$where = array('Games.status' => 1, 'Games.cid'=> array('IN',\$ids));
	}else {
		\$where = array('Games.status' => 1);
	}
    if (\$_keyword != '') {
		\$where['Games.title'] = array('like','%'.\$_keyword.'%');
	}
	if ($gtype) {
		\$where['Games.type'] = $gtype;
	}
	if ($line) {
		\$where['Games.line'] = $line;
	}
	if (!empty(\$_arcid)) {
		\$where['Games.id'] = array('IN', \$_arcid);
	}
	//分页
	if ($pagesize > 0) {

		\$count = D2('ArcView','Games')->where(\$where)->count();
		\$ename = I('e', '', 'htmlspecialchars,trim');
		if (!empty(\$ename) && C('URL_ROUTER_ON') == true) {
			\$param['p'] = I('p', 1, 'intval');
			\$param_action = '/'.\$ename;
	...
	\$_gamelist = D2('ArcView','Games')->nofield('$nofield')->where(\$where)->order("$orderby")->limit(\$limit)->select();
	...
	foreach(\$_gamelist as \$autoindex => \$gamelist):

	\$_jumpflag = (\$gamelist['flag'] & B_JUMP) == B_JUMP? true : false;
	\$gamelist['url'] = get_content_url(\$gamelist['id'], \$gamelist['cid'], \$gamelist['ename'], \$_jumpflag, \$gamelist['jumpurl']);

	if($titlelen) \$gamelist['title'] = str2sub(\$gamelist['title'], $titlelen, 0);
	if(\$gamelist['money']) \$gamelist['money'] = number_format(\$gamelist['money']);
...
str;
    ...
    //活动列表
    public function _activitylist($attr, $content) {
        ////非debug参属性参数只处理 一次
        $typeid   = !isset($attr['typeid']) || $attr['typeid'] == '' ? -1 : trim($attr['typeid']); //-1后面自动获取
        $arcid    = empty($attr['arcid']) ? '' : $attr['arcid']; //新增加20140413
        $titlelen = empty($attr['titlelen']) ? 0 : intval($attr['titlelen']);
...
<?php
	...
	if (\$_typeid>0 || substr(\$_typeid,0,1) == '$') {
		...
		\$ids = Common\Lib\Category::getChildsId(get_category(), \$_typeid, true);
		//p(\$ids);
		\$where = array('Activity.status' => 1, 'Activity.cid'=> array('IN',\$ids));
	}else {
		\$where = array('Activity.status' => 1);
	}
    if (\$_keyword != '') {
		\$where['Activity.title'] = array('like','%'.\$_keyword.'%');
	}
	if(\$over) {
		\$where['Activity.end_time'] = array('elt', time());
        \$where['Activity.type'] = array('neq', 1);
	}

	if (!empty(\$_arcid)) {
		\$where['Activity.id'] = array('IN', \$_arcid);
	}
	//分页
	if ($pagesize > 0) {

		\$count = D2('ArcView','Activity')->where(\$where)->count();
		\$ename = I('e', '', 'htmlspecialchars,trim');
		if (!empty(\$ename) && C('URL_ROUTER_ON') == true) {
			\$param['p'] = I('p', 1, 'intval');
			\$param_action = '/'.\$ename;
	...
	\$_activitylist = D2('ArcView','Activity')->nofield('$nofield')->where(\$where)->order("$orderby")->limit(\$limit)->select();

	if (empty(\$_activitylist)) {
		\$_activitylist = array();
	}


	foreach(\$_activitylist as \$autoindex => \$activitylist):

	\$activitylist['url'] = get_content_url(\$activitylist['id'], \$activitylist['cid'], \$activitylist['ename'], \$_jumpflag, \$activitylist['jumpurl']);
	if($titlelen) \$activitylist['title'] = str2sub(\$activitylist['title'], $titlelen, 0);
	\$activitylist['ctime'] = time();
...
str;
    ...
    //活动列表
    public function _bandlist($attr, $content) {
        ////非debug参属性参数只处理 一次
        $typeid   = !isset($attr['typeid']) || $attr['typeid'] == '' ? -1 : trim($attr['typeid']); //-1后面自动获取
        $arcid    = empty($attr['arcid']) ? '' : $attr['arcid']; //新增加20140413
        $titlelen = empty($attr['titlelen']) ? 0 : intval($attr['titlelen']);
...
<?php
	...
	if (\$_typeid>0 || substr(\$_typeid,0,1) == '$') {
		...
		\$ids = Common\Lib\Category::getChildsId(get_category(), \$_typeid, true);
		//p(\$ids);
		\$where = array('bands.status' => 1, 'bands.cid'=> array('IN',\$ids));
	}else {
		\$where = array('bands.status' => 1);
	}
    if (\$_keyword != '') {
		\$where['bands.title'] = array('like','%'.\$_keyword.'%');
	}

	if (!empty(\$_arcid)) {
		\$where['bands.id'] = array('IN', \$_arcid);
	}
	//分页
	if ($pagesize > 0) {

		\$count = D2('ArcView','bands')->where(\$where)->count();
		\$ename = I('e', '', 'htmlspecialchars,trim');
		if (!empty(\$ename) && C('URL_ROUTER_ON') == true) {
			\$param['p'] = I('p', 1, 'intval');
			\$param_action = '/'.\$ename;
	...
	\$_bandlist = D2('ArcView','bands')->nofield('$nofield')->where(\$where)->order("$orderby")->limit(\$limit)->select();
	...
	foreach(\$_bandlist as \$autoindex => \$bandlist):

	\$bandlist['url'] = get_content_url(\$bandlist['id'], \$bandlist['cid'], \$bandlist['ename'], \$_jumpflag, \$bandlist['jumpurl']);
	if($titlelen) \$bandlist['title'] = str2sub(\$bandlist['title'], $titlelen, 0);
	\$bandlist['ctime'] = time();
...
str;
    ...
    public function _catlist($attr, $content)
    {
        ...
        $attr['gtype'] && $gtype = $attr['gtype'];
        $attr['pid'] && $pid = intval($attr['pid']);
        $str     = <<<str
<?php
	...
	\$catlist['url'] = get_url(\$catlist, $pid, \$params);
?>
str;
