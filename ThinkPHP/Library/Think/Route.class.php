...
class Route {
    ...
    private static function _xss_check() {
        $_Securityconfig = get_cfg_value();
        $SAFE_LOG_POWER = $_Securityconfig['SAFE_LOG_POWER'];
        static $check = array('"', '>', '<', '\'', '(', ')', 'CONTENT-TRANSFER-ENCODING');
        if(isset($_GET['formhash']) && $_GET['formhash'] !== self::formhash()) {
            $SAFE_LOG_POWER && Safe::logFile(null,  __FUNCTION__);
            E('GoldWaf识别为恶意请求，请用正常方式体验网站(⊙o⊙)!');
            exit();
        }
    ...
    private static function checkUrlMatch($regx,$rule) {
        ...
        {
            $_Securityconfig = get_cfg_value();
            $URL_XSS_DEFEND = $_Securityconfig['URL_XSS_DEFEND'];
            if($URL_XSS_DEFEND) self::_xss_check();
        }
        $m1 = explode('/',$regx);
        $m2 = explode('/',$rule);
        $var = array();         
